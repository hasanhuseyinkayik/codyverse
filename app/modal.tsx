...
import {
  ...
  TouchableOpacity, Dimensions, Image, Linking, ActivityIndicator
} from 'react-native';
      ...
      {activeTab === 'lesson' && (
        <ScrollView style={styles.content} contentContainerStyle={styles.contentPadding}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push({ pathname: '/editor', params: { id: lesson.order } })}
          >
      ...
      {activeTab === 'video' && hasVideo && (
        <View style={styles.content}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              const videoId = lesson.video_url.split('/embed/')[1];
              Linking.openURL(`https://www.youtube.com/watch?v=${videoId}`);
            }}
          >
            <Image
              source={{ uri: `https://img.youtube.com/vi/${lesson.video_url.split('/embed/')[1]}/hqdefault.jpg` }}
          ...
          <Text style={styles.videoHint}>💡 YouTube uygulaması üzerinden oynatılır.</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  tabBar: { flexDirection: 'row', backgroundColor: '#1E1E2E', borderBottomWidth: 1, borderBottomColor: '#333' },
  ...
  challengeHeader: { flexDirection: 'row', alignItems: 'center', marginVertical: 20, gap: 10 },
  challengeDivider: { flex: 1, height: 1, backgroundColor: '#333' },
  challengeHeaderText: { color: '#00FFCC', fontSize: 16, fontWeight: 'bold' },
  challengeBox: { backgroundColor: '#1E1E2E', borderRadius: 12, padding: 16, borderLeftWidth: 4, borderLeftColor: '#00FFCC', marginBottom: 24 },
  challengeTitle: { color: '#00FFCC', fontSize: 16, fontWeight: 'bold', marginBottom: 8 },
  challengeText: { color: '#CCC', fontSize: 14, lineHeight: 22 },
  codeButton: { backgroundColor: '#00FFCC', borderRadius: 12, padding: 16, alignItems: 'center' },
  ...
  videoWrapper: { marginHorizontal: 16, borderRadius: 12, overflow: 'hidden', backgroundColor: '#000' },
  playOverlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.3)' },
  playButton: { width: 64, height: 64, borderRadius: 32, backgroundColor: 'rgba(0,0,0,0.75)', justifyContent: 'center', alignItems: 'center' },
